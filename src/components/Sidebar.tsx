...
const Sidebar = ({ isOpen, setIsOpen }: SidebarProps) => {
  ...
  const menuItems = [
    ...
    { name: "Suplementos - Categorías", icon: Package, path: "#" }, // pendiente
    { name: "Gastos", icon: DollarSign, path: "/expenses" }, // ✅ ya apunta a /expenses
    { name: "Producción", icon: Activity, path: "/production" },
    { name: "Novedades", icon: Bell, path: "/novelties" },
  ];

  const handleNavigate = (path: string) => {
    // Si aún no hay ruta real, no navegamos
    if (path === "#") return;
